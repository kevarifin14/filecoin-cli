...
describe('commands', () => {
  ...
  describe('retrieve', () => {
    test('JSON content output', async () => { const cmd = makeRetrieveCommand(); await cmd.parseAsync(['--cid', 'QmTest'], { from: 'user' }); const o = JSON.parse(stdoutOutput.join('')); expect(o.kind).toBe('filecoin_retrieve_result'); expect(o.cid).toBe('QmTest'); expect(o.content).toBe('hello world'); });
    test('JSON content output with parsed JSON payload', async () => { mockFetchIpfs.mockResolvedValue('{"name":"test"}'); const cmd = makeRetrieveCommand(); await cmd.parseAsync(['--cid', 'QmTest'], { from: 'user' }); const o = JSON.parse(stdoutOutput.join('')); expect(o.content.name).toBe('test'); });
    test('resolve-only output', async () => { const cmd = makeRetrieveCommand(); await cmd.parseAsync(['--cid', 'QmTest', '--resolve-only'], { from: 'user' }); const o = JSON.parse(stdoutOutput.join('')); expect(o.kind).toBe('filecoin_retrieve_plan'); expect(o.mode).toBe('resolve'); expect(o.sizeHuman).toContain('KB'); });
    test('resolve-only output with unknown metadata', async () => { mockResolveIpfs.mockResolvedValue({ url: 'https://ipfs.io/ipfs/Qm', contentType: null, size: null }); const cmd = makeRetrieveCommand(); await cmd.parseAsync(['--cid', 'QmTest', '--resolve-only'], { from: 'user' }); const o = JSON.parse(stdoutOutput.join('')); expect(o.sizeHuman).toBeNull(); expect(o.contentType).toBeNull(); });
    test('resolve-only pretty output with unknown fields', async () => { mockResolveIpfs.mockResolvedValue({ url: 'https://ipfs.io/ipfs/Qm', contentType: null, size: null }); const cmd = makeRetrieveCommand(); await cmd.parseAsync(['--cid', 'QmTest', '--resolve-only', '--pretty'], { from: 'user' }); expect(stdoutOutput.join('\n')).toContain('Retrieve (resolve): QmTest'); expect(stdoutOutput.join('\n')).toContain('(unknown)'); });
    test('resolve-only pretty output with known fields', async () => { const cmd = makeRetrieveCommand(); await cmd.parseAsync(['--cid', 'QmTest', '--resolve-only', '--pretty'], { from: 'user' }); expect(stdoutOutput.join('\n')).toContain('text/plain'); expect(stdoutOutput.join('\n')).toContain('1.00 KB'); });
    test('pretty output', async () => { const cmd = makeRetrieveCommand(); await cmd.parseAsync(['--cid', 'QmTest', '--pretty'], { from: 'user' }); expect(stdoutOutput.join('\n')).toContain('Retrieve: QmTest'); expect(stdoutOutput.join('\n')).toContain('text/plain'); expect(stdoutOutput.join('\n')).toContain('1.00 KB'); });
    test('pretty output with unknown metadata and text payload', async () => { mockResolveIpfs.mockResolvedValue({ url: 'https://ipfs.io/ipfs/Qm', contentType: null, size: null }); const cmd = makeRetrieveCommand(); await cmd.parseAsync(['--cid', 'QmTest', '--pretty'], { from: 'user' }); expect(stdoutOutput.join('\n')).toContain('(unknown)'); expect(stdoutOutput.join('\n')).toContain('hello world'); });
    test('pretty output with parsed JSON payload', async () => { mockFetchIpfs.mockResolvedValue('{"name":"test"}'); const cmd = makeRetrieveCommand(); await cmd.parseAsync(['--cid', 'QmTest', '--pretty'], { from: 'user' }); expect(stdoutOutput.join('\n')).toContain('"name": "test"'); });
    test('pretty output with long text truncates', async () => { mockFetchIpfs.mockResolvedValue('x'.repeat(3001)); const cmd = makeRetrieveCommand(); await cmd.parseAsync(['--cid', 'QmTest', '--pretty'], { from: 'user' }); expect(stdoutOutput.join('\n')).toContain('3001 bytes total'); });
    ...
    test('unknown error output', async () => { mockFetchIpfs.mockRejectedValue(undefined); const cmd = makeRetrieveCommand(); await cmd.parseAsync(['--cid', 'QmTest'], { from: 'user' }); expect(JSON.parse(stderrOutput[0]).code).toBe('UNKNOWN'); });
  });
  ...
  describe('store', () => {
    ...
    test('pretty output without storage context', async () => { const cmd = makeStoreCommand(); await cmd.parseAsync(['--from', 'f1from', '--to', 'f01234', '--method', '2', '--params', 'AQI=', '--pretty'], { from: 'user' }); expect(stdoutOutput.join('\n')).toContain('(none)'); });
    ...
    test('unknown error output', async () => { mockGetNetworkName.mockRejectedValue(undefined); const cmd = makeStoreCommand(); await cmd.parseAsync(['--from', 'f1from', '--to', 'f01234', '--method', '2', '--params', 'AQI='], { from: 'user' }); expect(JSON.parse(stderrOutput[0]).code).toBe('UNKNOWN'); });
  });
});
